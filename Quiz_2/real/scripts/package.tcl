...
#   * Adds a Sub-Core Reference to xilinx.com:ip:c_addsub:12.0 under both
#     the VHDL Synthesis and VHDL Simulation file groups - this is what
#     the textbook calls out (Digital Systems Projects p. 445) and is what
#     pulls the IP-Catalog block into the packaged IP correctly.
...
set_property description  "32-bit signed adder/subtractor (c_addsub v12.0) with B held to constant -1 (all ones) and Latency=0; wrapped as an AXI4-Lite slave so software writes X and reads back z = x - 1." $core
...
foreach fg {xilinx_vhdlsynthesis xilinx_vhdlbehavioralsimulation} {
    ...
    set existing [ipx::get_subcore_references xilinx.com:ip:c_addsub:12.0 \
                      -of_objects $group -quiet]
    if {[llength $existing] == 0} {
        puts "== Adding sub-core reference xilinx.com:ip:c_addsub:12.0 to $fg"
...
puts "== add_const_axi metadata saved.  Now click:"
